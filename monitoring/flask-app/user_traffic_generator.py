...
TARGET_APP_URL = "http://application-sg-1:8000"
...
def call_url():
    ...
    return jsonify({"message": "Traffic generation started for all endpoints"}), 200
...
def generate_random_data(endpoint):
    ...
    elif endpoint == "/io_task":
        return endpoint

    elif endpoint == "/cpu_task":
        return endpoint
    ...
    elif endpoint == "/random_sleep":
        return endpoint

    elif endpoint == "/error_test":
        return endpoint
...
@app.route('/')
def home():
    return jsonify({"message": "Traffic generator is running. Use /call_url to start traffic generation."}), 200
