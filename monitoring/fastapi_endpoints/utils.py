...
class PrometheusMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: RequestResponseEndpoint) -> Response:
        ...
        REQUESTS.labels(method=method, path=path, app_name=self.app_name).inc()     #Increments requests_in_progress gauge when a new request is processed 
        ...
        try:
            ...
            REQUESTS_IN_PROGRESS.labels(
                method=method, path=path, app_name=self.app_name).dec()             #Decrements the gauge when request finishes processing, successfully/with error
...
def setting_otlp(app: ASGIApp, app_name: str, endpoint: str, log_correlation: bool = True) -> None:
    ...
    batch_processor = BatchSpanProcessor(
        ...
        max_export_batch_size=1,       # Set smaller batch size for frequent exports
        schedule_delay_millis=500       # Set shorter delay in ms to export more often
    )
